...
for(const name of ['guide-base.js','guide-itinerary.js','guide-weather.js','guide-media.js','guide-hotel-media.js','guide-extra-media.js'])vm.runInContext(read(name),c);
...
for(const n of c.window.TIBET_WEATHER.nodes){assert.ok(n.sourceUrl.startsWith('https://tianqi.moji.com/'));if(n.date>'2026-10-03'){assert.equal(n.high,null);assert.equal(n.low,null);}}
...
for(const width of [360,390,768,1440]){
  ...
  for(const n of merged){if(n.members.length>1){assert.ok(n.members.every(m=>m.date===n.date));assert.ok(Math.max(...n.members.map(m=>m.high))-Math.min(...n.members.map(m=>m.high))<=2);assert.ok(Math.max(...n.members.map(m=>m.low))-Math.min(...n.members.map(m=>m.low))<=2);assert.equal(n.high,Math.max(...n.members.map(m=>m.high)));assert.equal(n.low,Math.min(...n.members.map(m=>m.low)));}}
  assert.equal(merged.flatMap(n=>n.members).length,36);
